...
if (args.length === 1) {
  if (args[0] === '--help') {
    ...
    (0, _index.validateAndStats)(path, options).then(resp => resp.forEach(values => console.log(` Path: ${values.file}\n Link: ${values.href}\n Text: ${values.text}\n`))).catch(err => err);
  }
}
...
if (args.length === 2) {
  if (args[1] === '--validate' || args[1] === '--v') {
    ...
    options.stats = true;
    (0, _index.validateAndStats)(path, options).then(resp => resp.forEach(values => console.log(values))).catch(err => console.log(err));
  }
}
